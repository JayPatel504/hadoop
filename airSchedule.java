...
public class airSchedule {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, LongWritable> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (isInt(col[14]) && isInt(col[15])) {
				if (Integer.parseInt(col[14]) >= 10 && Integer.parseInt(col[15]) >= 10) {
					Text dC = new Text("* " + f);
					context.write(dC, temp);
				}
				Text totF = new Text(f);
				context.write(totF, temp);
			}
		}
	}
	...
	public static class Red extends Reducer<Text, LongWritable, Text, Text> {
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			context.write(new Text("Airline with Highest delay:  "), null);
			while (!high.isEmpty()) {
				Dict pair = high.pollFirst();
				context.write(new Text(pair.key), new Text(Double.toString(pair.val)));
			}
			context.write(new Text("Airline with Lowest delay:  "), null);
			while (!low.isEmpty()) {
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "Airline Schedule");
		job.setJarByClass(airSchedule.class);
